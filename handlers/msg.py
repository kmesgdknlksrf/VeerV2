...
def updateHandlers(client, message,redis):
	...
	if type is "private" and message.outgoing != True:
		...
		if text and re.search("^/start$",text):
			...
			if rank == "sudo":
				kb = ReplyKeyboardMarkup([[r.RKgp, r.RKgpl],[r.RKaf, r.RKrf],[r.RKf],["جلب نسخة احتياطيه"],[r.RKub]],resize_keyboard=True)
			...
			kb = InlineKeyboardMarkup([[InlineKeyboardButton("My Channel", url="t.me/"+yousef_labban1)]])
